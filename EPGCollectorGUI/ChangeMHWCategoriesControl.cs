...
namespace EPGCentre
{
    public partial class ChangeMHWCategoriesControl : UserControl, IUpdateControl
    {
        /// <summary>
        /// Get the general window heading for the data.
        /// </summary>
        public string Heading { get { return ("EPG Centre - Change MHW Program Categories - "); } }
        /// <summary>
        ...
        /// </summary>
        public string DefaultFileName { get { return ("MHWn Categories fffff - x"); } }
        /// <summary>
        /// Get the save file filter.
        /// </summary>
        public string SaveFileFilter { get { return ("MHW Program Category Files (MHW*.cfg)|MHW*.cfg"); } }
        /// <summary>
        /// Get the save file title.
        /// </summary>
        public string SaveFileTitle { get { return ("Save EPG Collection MHW Program Category File"); } }
        /// <summary>
        ...
        private BindingList<MediaHighwayProgramCategory> bindingList;
        ...
        public ChangeMHWCategoriesControl()
        {
            InitializeComponent();
        }
        ...
        public bool Process(string fileName)
        {
            if (fileName != null)
            {
                bool reply = MediaHighwayProgramCategory.Load(fileName);
                if (!reply)
                    return (false);
            }
            ...
            bindingList = new BindingList<MediaHighwayProgramCategory>();
            foreach (MediaHighwayProgramCategory category in MediaHighwayProgramCategory.Categories)
                bindingList.Add(new MediaHighwayProgramCategory(category.CategoryID, category.FullDescription));
        ...
        private void dgCategoriesRowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (MediaHighwayProgramCategory category in bindingList)
                {
        ...
        private void dgCategoriesColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            Collection<MediaHighwayProgramCategory> sortedCategories = new Collection<MediaHighwayProgramCategory>();
            ...
            bindingList = new BindingList<MediaHighwayProgramCategory>();
            foreach (MediaHighwayProgramCategory category in sortedCategories)
                bindingList.Add(category);

            categoryBindingSource.DataSource = bindingList;
        }

        private void addInOrder(Collection<MediaHighwayProgramCategory> categories, MediaHighwayProgramCategory newCategory, bool sortedAscending, string keyName)
        {
            sortedKeyName = keyName;

            foreach (MediaHighwayProgramCategory oldCategory in categories)
            {
                if (sortedAscending)
                {
        ...
        private DataState hasDataChanged()
        {
            ...
            if (bindingList.Count != MediaHighwayProgramCategory.Categories.Count)
                return (DataState.Changed);

            foreach (MediaHighwayProgramCategory category in bindingList)
            {
                MediaHighwayProgramCategory existingCategory = MediaHighwayProgramCategory.FindCategory(category.CategoryID);
        ...
        public bool PrepareToSave()
        {
            ...
            {
                MediaHighwayProgramCategory category = bindingList[0];

                if (category.CategoryID == 0 &&
                    category.FullDescription == string.Empty)
                {
                    MessageBox.Show("No categories defined.", "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return (false);
                }
            ...
            Collection<MediaHighwayProgramCategory> addedList = new Collection<MediaHighwayProgramCategory>();
            ...
            foreach (MediaHighwayProgramCategory category in bindingList)
            {
                ...
                addedList.Add(new MediaHighwayProgramCategory(category.CategoryID, category.FullDescription));
            }

            MediaHighwayProgramCategory.Categories.Clear();

            foreach (MediaHighwayProgramCategory category in bindingList)
                MediaHighwayProgramCategory.AddCategory(category.CategoryID, category.FullDescription);

            return (true);
        }

        private bool validateEntry(Collection<MediaHighwayProgramCategory> addedList, MediaHighwayProgramCategory category)
        {
            foreach (MediaHighwayProgramCategory existingCategory in addedList)
            {
        ...
        public bool Save(string fileName)
        {
            ...
            string message = MediaHighwayProgramCategory.Save(fileName);

            if (message == null)
            {
                MessageBox.Show("The MediaHighway program categories have been saved to '" + fileName + "'", " EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Information);
                currentFileName = fileName;
            }
            else
                MessageBox.Show("An error has occurred while writing the MediaHighway program categories." + Environment.NewLine + Environment.NewLine + message, "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
