...
namespace EPGCentre
{
    public partial class ChangePSIPCategoriesControl : UserControl, IUpdateControl
    {
        /// <summary>
        /// Get the general window heading for the data.
        /// </summary>
        public string Heading { get { return ("EPG Centre - Change PSIP Program Categories - "); } }
        /// <summary>
        ...
        /// </summary>
        public string DefaultFileName { get { return ("ATSC PSIP Categories"); } }
        /// <summary>
        /// Get the save file filter.
        /// </summary>
        public string SaveFileFilter { get { return ("PSIP Program Category Files (ATSC PSIP Categories*.cfg)|ATSC PSIP Categories*.cfg"); } }
        /// <summary>
        /// Get the save file title.
        /// </summary>
        public string SaveFileTitle { get { return ("Save EPG Collection ATSC PSIP Program Category File"); } }
        /// <summary>
        ...
        private BindingList<AtscPsipProgramCategory> bindingList;
        ...
        public ChangePSIPCategoriesControl()
        {
            InitializeComponent();
        }
        ...
        {
            bool reply = AtscPsipProgramCategory.Load(fileName);
            if (!reply)
                return (false);

            currentFileName = fileName;

            bindingList = new BindingList<AtscPsipProgramCategory>();
            foreach (AtscPsipProgramCategory category in AtscPsipProgramCategory.Categories)
                bindingList.Add(new AtscPsipProgramCategory(category.CategoryID, category.FullDescription));
        ...
        private void dgCategoriesRowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            try
            {
                int categoryID = Int32.Parse(dgCategories.Rows[e.RowIndex].Cells["categoryColumn"].Value.ToString().Trim());

                foreach (AtscPsipProgramCategory category in bindingList)
                {
            ...
            {
                MessageBox.Show("Category ID '" + categoryIDString + "' is incorrect.", "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
                e.Cancel = true;
                return;
            }
            ...
            {
                MessageBox.Show("The PSIP description for category " + categoryIDString + " is incorrect.", "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
                e.Cancel = true;
                return;
            }
        ...
        private void dgCategoriesColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            Collection<AtscPsipProgramCategory> sortedCategories = new Collection<AtscPsipProgramCategory>();
            ...
            bindingList = new BindingList<AtscPsipProgramCategory>();
            foreach (AtscPsipProgramCategory category in sortedCategories)
                bindingList.Add(category);

            categoryBindingSource.DataSource = bindingList;
        }

        private void addInOrder(Collection<AtscPsipProgramCategory> categories, AtscPsipProgramCategory newCategory, bool sortedAscending, string keyName)
        {
            sortedKeyName = keyName;

            foreach (AtscPsipProgramCategory oldCategory in categories)
            {
                if (sortedAscending)
                {
        ...
        private DataState hasDataChanged()
        {
            ...
            if (bindingList.Count != AtscPsipProgramCategory.Categories.Count)
                return (DataState.Changed);

            foreach (AtscPsipProgramCategory category in bindingList)
            {
                AtscPsipProgramCategory existingCategory = AtscPsipProgramCategory.FindCategory(category.CategoryID);
        ...
        public bool PrepareToSave()
        {
            ...
            Collection<AtscPsipProgramCategory> addedList = new Collection<AtscPsipProgramCategory>();
            ...
            foreach (AtscPsipProgramCategory category in bindingList)
            {
                ...
                addedList.Add(new AtscPsipProgramCategory(category.CategoryID, category.FullDescription));
            }

            AtscPsipProgramCategory.Categories.Clear();

            foreach (AtscPsipProgramCategory category in bindingList)
                AtscPsipProgramCategory.AddCategory(category.CategoryID, category.FullDescription);

            return (true);
        }

        private bool validateEntry(Collection<AtscPsipProgramCategory> addedList, AtscPsipProgramCategory category)
        {
            foreach (AtscPsipProgramCategory existingCategory in addedList)
            {
            ...
            {
                MessageBox.Show("The PSIP description for category " + category.CategoryIDString + " is incorrect.", "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return (false);
            }
        ...
        public bool Save(string fileName)
        {
            ...
            string message = AtscPsipProgramCategory.Save(fileName);

            if (message == null)
            {
                MessageBox.Show("The PSIP program categories have been saved to '" + fileName + "'", " EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Information);
                currentFileName = fileName;
            }
            else
                MessageBox.Show("An error has occurred while writing the PSIP program categories." + Environment.NewLine + Environment.NewLine + message, "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
